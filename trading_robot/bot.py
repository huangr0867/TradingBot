...
from trading_robot.portfolio import Portfolio
from trading_robot.stockframe import StockFrame
from trading_robot.trades import Trade
...
class TradeBot():
    ...
    def pre_market_open(self) -> bool:
        ...
        right_now = datetime.utcnow().replace(tzinfo=timezone.utc).timestamp()
    ...
    def regular_market_open(self) -> bool:
        market_start_time = datetime.utcnow().replace(hour=13, minute=00, second=00).timestamp()
        market_end_time = datetime.utcnow().replace(hour=20, minute=00, second=00).timestamp()
        right_now = datetime.utcnow().replace(tzinfo=timezone.utc).timestamp()
    ...
    def create_portfolio(self):
        ...
        # assign the client (TD Ameritrade)
