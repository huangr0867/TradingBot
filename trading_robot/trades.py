...
class Trade():
    ...
    def modify_side(self, side: Optional(str), order_leg_id: int = 0) -> None:
        ...
        if side:
            ...
            self.side['orderLegCollection'][order_leg_id]['instructions'] = self.order_instructions[self.enter_or_exit][self.side_opposite]
    ...
    def add_stop_loss(self, stop_size: float, percentage: bool = False) -> bool:
        ...
        if percentage:
            ...
            new_price = self._calculate_new_orice(price=price, adjustment=adjustment, percentage=False)
