...
class Indicators():
    ...
    def set_indicator_signals(self, indicator: str, buy: float, sell: float, condition_buy: Any, condition_sell: Any) -> None:
        # if there's no signal, set it to an empty dict
    ...
    def get_indicator_signal(self, indicator: Optional[str]) -> dict:
        # if want to see a specific indicator (and indicator is inputted)
        if indicator and indicator in self._indicator_signals:
            return self._indicator_signals[indicator]
        
        else:
            # else it will return the entire indicator signals
    ...
    def rsi(self, period: int, method: str = 'wilders') -> pd.DataFrame:
        ...
        # define the up days
        # if the change in price is pos (>= 0), return x else 0
    ...
    def check_signals(self) -> Union[pd.Series, None]:
        signals_df = self._stock_frame(indicators=self._indicator_signals)
